...
import { Calendar, Briefcase, ChevronLeft, RefreshCw, Clock, CheckCircle, XCircle, Info, UserCircle, ChevronDown, ChevronRight, FileText, PlusCircle, Check } from 'lucide-react';
...
function Dashboard({ userId, userRole }: { userId?: string, userRole?: string }) {
  ...
  const updateStatus = async (id: number, status: string) => {
    try {
      await axios.patch(`http://localhost:3001/api/requests/${id}/status`, { status, managerId: userId });
      fetchRequests();
                ...
                <table className="w-full text-left">
                  <thead className="bg-[#1a1b2e] text-[9px] text-slate-500 uppercase font-bold border-b border-slate-700">
                    <tr>
                      ...
                      <th className="px-6 py-4">Event / Department</th>
                      ...
                      <React.Fragment key={req.id}>
                        <tr className="hover:bg-slate-700/20">
                          ...
                          <td className="px-6 py-4 text-sm text-slate-300">
                            ...
                            <div className="text-[10px] uppercase font-bold text-slate-500 tracking-wider mt-1">{req.department}</div>
                          </td>
                          ...
                          {isManager && (
                            <td className="px-6 py-4 text-center"><div className="flex justify-center gap-2">
                              <button onClick={() => updateStatus(req.id, 'Approved')} className="p-1 text-emerald-500 hover:bg-emerald-500 hover:text-white rounded border border-emerald-500/20"><CheckCircle size={16} /></button>
                              <button onClick={() => updateStatus(req.id, 'Need More Info')} className="p-1 text-amber-500 hover:bg-amber-500 hover:text-white rounded border border-amber-500/20"><Info size={16} /></button>
                              <button onClick={() => updateStatus(req.id, 'Rejected')} className="p-1 text-rose-500 hover:bg-rose-500 hover:text-white rounded border border-rose-500/20"><XCircle size={16} /></button>
                            </div></td>
                          )}
                        </tr>
                        {expandedId === req.id && (
                          <tr className="bg-[#1a1b2e]/60">
                            <td colSpan={isManager ? 6 : 4} className="p-8">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <div className="flex gap-2 mt-4">
                                      <input 
                                        ...
                                        placeholder="Add custom expense (e.g. Visa Fee)"
                                        className="flex-1 bg-[#1a1b2e] border border-slate-700 rounded-lg px-3 py-2 text-xs outline-none focus:border-indigo-500"
                                      />
                                      <button onClick={() => addCustomTask(req.id)} className="bg-slate-700 p-2 rounded-lg hover:bg-indigo-600 transition-colors">
                                        <PlusCircle size={18} />
                                      </button>
                                    </div>
                                  </div>
                                ...
                                <div className="space-y-4">
                                  <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Internal Notes</h4>
                                  <p className="text-sm text-slate-400 bg-[#242645] p-4 rounded-lg border border-slate-700/50 italic leading-relaxed whitespace-pre-wrap">{req.notes || 'No notes provided.'}</p>
                                </div>
                              </div>
                            </td>
                          </tr>
